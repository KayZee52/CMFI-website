...
<div class="page-content" style="padding: 32px; max-width: 1000px; margin: 0 auto;" x-data="{ editing: false }">
    <div class="page-header" style="margin-bottom: 40px; display: flex; justify-content: space-between; align-items: flex-end;">
        ...
        <div class="flex items-center gap-3">
            @if (session('status') === 'profile-updated' || session('status') === 'password-updated')
                <div x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)" style="display: flex; align-items: center; gap: 8px; background: #ECFDF5; color: #065F46; padding: 10px 16px; border-radius: 12px; font-size: 14px; font-weight: 600; border: 1px solid #A7F3D0;">
            ...
            @endif
            <button type="button" @click="editing = !editing" class="btn-secondary" style="height: 44px; padding: 0 20px; border-radius: 12px; font-weight: 700; display: flex; align-items: center; gap: 8px; border: 1.5px solid #E2E8F0; background: #fff;">
    ...
    <form id="profile-info-form" method="post" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('patch')
        ...
        <div class="profile-card" style="display: grid; grid-template-columns: 300px 1fr; gap: 48px; align-items: start;">
                ...
                <div style="position: relative; display: inline-block; margin-bottom: 20px;">
                    ...
                    <label for="avatar-input" style="position: absolute; bottom: -10px; right: -10px; width: 44px; height: 44px; background: var(--primary); border-radius: 14px; display: flex; align-items: center; justify-content: center; cursor: pointer; color: white; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06); transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                        ...
                        <input type="file" id="avatar-input" name="avatar" class="hidden" accept="image/*" 
                               onchange="document.getElementById('avatar-preview').src = window.URL.createObjectURL(this.files[0]); editing = true;">
                    </label>
                </div>
            ...
            <div class="space-y-6">
                ...
                <div class="form-group">
                    ...
                    <input x-show="editing" id="name" name="name" type="text" class="form-input" value="{{ old('name', $user->name) }}" required />
                </div>
                ...
                <div style="padding-top: 24px; border-top: 1px solid #F1F5F9; margin-top: 24px;">
                    <button type="submit" class="btn-primary" style="height: 52px; padding: 0 32px; border-radius: 14px; font-weight: 800; font-size: 15px; box-shadow: 0 10px 15px -3px rgba(37, 99, 235, 0.3); display: inline-flex; align-items: center; justify-content: center; gap: 10px; width: auto; white-space: nowrap;">
                        <x-icon name="check" style="width: 20px; height: 20px;" />
                        Save Profile Changes
                    </button>
                    <p x-show="!editing" style="font-size: 13px; color: #64748B; margin-top: 12px; font-weight: 500; display: flex; align-items: center; gap: 6px;">
                        <x-icon name="help" style="width: 14px; height: 14px; color: #94A3B8;" />
        ...
        <div style="max-width: 500px;">
            <form id="password-update-form" method="post" action="{{ route('password.update') }}" class="space-y-6">
                ...
                <div class="form-group">
                    ...
                    <input name="current_password" type="password" class="form-input" placeholder="••••••••" required />
                ...
                <button type="submit" class="btn-primary" style="height: 48px; border-radius: 14px; font-weight: 700; background: #0F172A;">
                    Update Security Key
                </button>
            </form>
        </div>
